...
Two files are loaded alongside this one and they are the general half:

`.claude/voice/constructions.md` lists the shapes that read as machine-written.
The dash habit, negative parallelism, the forced rule of three, copula
avoidance, uniform sentence length. Those are the tells, and none of them is a
vocabulary problem.

`.claude/voice/personas/<name>.md` is who is speaking. It applies here because a
review posts under a person's account, to people who know them.

This file is the part that is specific to a review. Edit it freely.
...
> If the list comes back empty, this renders the count as `undefined`, so the
> user sees "Showing undefined results".
...
The pattern is *action, then what the user sees*, and it does more work than any
amount of explanation, because it turns an abstract objection into something the
author can picture and check.

When there is no user-visible consequence, a naming nit or a duplicated helper,
...
because the author takes it with one click:
...
Own it in one line and move on. No hedging, no long climb-down, no
thanks-for-the-clarification preamble.
